...
public:
  ...
  : Node("scan_shadow_filter_node", options)
  {
    ...
    angle_min_deg_ = this->declare_parameter<double>("angle_min_deg", -85.0);
    angle_max_deg_ = this->declare_parameter<double>("angle_max_deg", 85.0);
    angle_min_rad_ = angle_min_deg_ * M_PI / 180.0;
    angle_max_rad_ = angle_max_deg_ * M_PI / 180.0;

    // Laser -> base_link の固定オフセット（必要なら調整）
...
private:
  ...
  void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
  {
    ...
    // (1) 影・外れ値の簡易除去（隣接レンジ差のしきい値）
    std::vector<bool> mask = compute_shadow_mask(ranges, threshold_);

    // (2) Laser frame → base_link frame へ
    std::vector<std::array<float,2>> pts_sel;   // 近傍半径を満たす点だけ格納
    ...
    for (size_t i = 0; i < ranges.size(); ++i) {
      ...
      const double xl = static_cast<double>(r) * std::cos(ang);
      const double yl = static_cast<double>(r) * std::sin(ang);

      // laser -> base_link (回転→並進)
      const double xb = cL*xl - sL*yl + laser_offset_x_;
      const double yb = sL*xl + cL*yl + laser_offset_y_;
    ...
    // === ④ Full Scan モードなら生点群を送る ===
    if (full_scan_mode_) {
        full_scan_mode_ = false;  // 1回だけ実施

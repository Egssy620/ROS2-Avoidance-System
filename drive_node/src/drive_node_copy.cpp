...
public:
  DriveNode() : Node("drive_node") {
    // ---- Parameters ----
    v_max_         = this->declare_parameter<double>("v_max",         0.25);  // m/s
    v_min_         = this->declare_parameter<double>("v_min",         0.05); // m/s
    slow_dist_     = this->declare_parameter<double>("slowdown_dist", 1.5);  // m
    stop_dist_     = this->declare_parameter<double>("stop_dist",     0.025); // m
    omega_max_     = this->declare_parameter<double>("omega_max",     0.20);  // rad/s
    ctrl_rate_hz_  = this->declare_parameter<double>("rate",          20.0); // Hz
    tau_ = this->declare_parameter("omega_filter_tau", 0.3);  // 例: 0.3s
    dt_  = this->declare_parameter("control_dt", 0.05);       // 例: 50ms周期
    ...
    pose_sub_ = this->create_subscription<geometry_msgs::msg::Pose2D>(
      "/robot_pose2d", 10, std::bind(&DriveNode::onPose, this, std::placeholders::_1));
...
private:
  ...
  void onPose(const geometry_msgs::msg::Pose2D::SharedPtr msg){
    xr_ = msg->x; yr_ = msg->y; thetar_ = msg->theta + M_PI / 2.0;; have_pose_ = true;
  }
  ...
  void onTimer(){
    ...
    // ---- goal distance for speed profile ----

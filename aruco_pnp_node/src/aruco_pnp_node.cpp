// aruco_pnp_node/src/aruco_pnp_node.cpp
// ArUcoマーカ検出とPnPによる自己位置推定ノード
...
public:
  ArUcoLocalizerNode() : Node("aruco_pnp_node"), steady_clock_(RCL_STEADY_TIME){
    ...
    yaw_debug_pub_ = this->create_publisher<std_msgs::msg::Float32MultiArray>(
    "/yaw_debug", 10);
...
private:
  // LiDAR yaw からの補正量 [rad]
  double delta_yaw_lidar_ = 0.0;
  // ==== cmd_vel 受信 ====
          ...
          const rclcpp::Time& t_now)
  {
...

      {
        ...
        // LiDAR補正yaw
        double yaw_lidar_est = wrapToPi(x_est_.yaw + delta_yaw_lidar_);
        dbg.data[1] = yaw_lidar_est;

        // 現在の推定yaw
        dbg.data[2] = x_est_.yaw;
  ...
  void workerLoop(){
    ...
    while (rclcpp::ok() && running_.load()) {
      ...
      for (size_t k=0;k<ids.size();++k){
        ...
        if (rms > RMS_THRESH_PX) continue;  // 粗いフィルタ
  ...
  rclcpp::Publisher<std_msgs::msg::Float32MultiArray>::SharedPtr yaw_debug_pub_;

...
ROS2 Avoidance System は、ROS2 上で動作する自律移動ロボット向けの障害物回避フレームワーク.
本プロジェクトは複数のノードで構成され、LiDAR センサを用いた距離情報処理、経路追従、姿勢調整、回避戦略を統合する．

本リポジトリは、実機およびシミュレーション環境での自律走行・障害物回避タスクに対応する設計になっている．

---

## 🔎 概要

- カメラによるArUcoマーカー認識に基づく自己位置推定
- LiDAR による障害物検知・回避
- 目標姿勢への誘導とモードベース制御
- 複数独立ノード構成による拡張性
- ROS2 Humble に対応

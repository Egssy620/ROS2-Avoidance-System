...
public:
  ManageNode() : Node("manage_node") {
    ...
    {
      rclcpp::QoS qos(1);
      qos.transient_local();        // ← これが超重要
      cmd_source_pub_ = this->create_publisher<std_msgs::msg::String>("/cmd_source", qos);
    }
    ...
    // === Subscriber ===
    pose2d_sub_ = create_subscription<geometry_msgs::msg::Pose2D>(
        "/robot_pose2d_fused", 10,
        std::bind(&ManageNode::onPose2D, this, std::placeholders::_1));

    lidar_sub_ = create_subscription<sensor_msgs::msg::PointCloud2>(
        "/filtered_points_map", 10,
        std::bind(&ManageNode::onPointsMap, this, std::placeholders::_1));

    fullscan_sub_ = create_subscription<sensor_msgs::msg::PointCloud2>(
        "/full_scan_points_map", 10,
...
private:
  ...
  void onPointsMap(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
      lidar_points_.clear();
      ...
      for (; iter_x != iter_x.end(); ++iter_x, ++iter_y) {
          lidar_points_.emplace_back(*iter_x, *iter_y);
      }
  }
  // ============================================================
  //                       フルスキャン要求

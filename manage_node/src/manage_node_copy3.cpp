...
public:
  ManageNode() : Node("manage_node") {
    ...
    // === パラメータ ===
    lookahead_dist_ = this->declare_parameter("lookahead_distance", 1.5);
    ...
    // === Subscriber ===
    pose_source_sub_ = this->create_subscription<std_msgs::msg::String>(
    "/pose_source", 10,
    std::bind(&ManageNode::onPoseSource, this, std::placeholders::_1));
...
private:
  ...
  bool isGoalBlocked()
  {
      ...
      const double R_GOAL = 1.5;   // ゴール周囲の判定半径
      const int MIN_POINTS = 5;    // ノイズ除去
  ...
  void onHeadingDone(const std_msgs::msg::Bool::SharedPtr msg)
  {
    ...
    if (phase_ == Phase::HEADING) {
      updatePathToCurrentGoal();     // 経路生成
      heading_sent_ = false;         // 新しい経路用の heading を許可
  ...
  void onDriveDone(const std_msgs::msg::Bool::SharedPtr msg)
  {
    ...
    // Goal0 に戻ってきた & スキップゴールあり → 障害物チェック
    ...
    // 通常：タスク用の back_yaw へ向く
    double back_yaw = -M_PI/2.0;     // タスク時に向かせたい姿勢
    std_msgs::msg::Float32 msg_yaw;
  ...
  void onPoseSource(const std_msgs::msg::String::SharedPtr msg)
  {
      static bool camera_notified = false;
      const std::string& s = msg->data;

      if (!camera_notified &&
          (s == "measured_initial" || s == "measured"))
      {
          playVoice("カメラ起動!");
          camera_notified = true;
      }

      if (s == "measured" || s == "measured_initial") {
          pose_state_ = PoseSourceState::MEASURED;        // 緑
      }
      else if (s == "blended") {
          pose_state_ = PoseSourceState::BLENDED;         // オレンジ
      }
      else if (s == "predicted") {
          pose_state_ = PoseSourceState::PREDICTED;       // 黄
      }
      else if (s == "reject_keep_prev") {
          pose_state_ = PoseSourceState::REJECT_KEEP_PREV; // 青 ← 新追加
      }
      else {
          pose_state_ = PoseSourceState::LOST;            // 赤
      }
  }
  ...
  void onPose2D(const geometry_msgs::msg::Pose2D::SharedPtr msg)
  {
    if (!received_pose_) {
        ...
        if (dt > 0.5) pose_state_ = PoseSourceState::LOST;
    }
    ...
    cv::Point2d new_pos(x_robot, y_robot);
    double new_yaw = yaw_cam + M_PI/2.0;

    // 移動平均
    const int N = 5;
    pos_buffer_.push_back(new_pos);
    yaw_buffer_.push_back(new_yaw);
    if (pos_buffer_.size() > (size_t)N) {
      pos_buffer_.erase(pos_buffer_.begin());
      yaw_buffer_.erase(yaw_buffer_.begin());
    }

    cv::Point2d mean_pos(0,0);
    double sx = 0, sy = 0;
    for (size_t i=0; i<pos_buffer_.size(); ++i) {
      mean_pos += pos_buffer_[i];
      sx += std::cos(yaw_buffer_[i]);
      sy += std::sin(yaw_buffer_[i]);
    }
    mean_pos *= 1.0 / pos_buffer_.size();
    double mean_yaw = std::atan2(sy, sx);
    ...
    if (!received_pose_) {
      received_pose_ = true;
      playVoice("自己位置の取得が完了しました。");

      updatePathToCurrentGoal();
      playVoice("経路生成が完了しました。");
    }
    ...
    // HEADING フェーズのとき最初の heading_target を送る
  ...
  void onLidar(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    static bool lidar_notified = false;
    ...
    for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z) {
        ...
        // ★ offset を入れない（scan_shadow_filter_node で済んでいる）
        double x_robot = x_local;
        double y_robot = y_local;

        // ロボットヨー角で map に変換
        double x_map = c * x_robot - s * y_robot + cx;
        double y_map = s * x_robot + c * y_robot + cy;
  ...
  void startObstacleScan()
  {
    ...
    // スキャン中は cmd_source を none にして停止させる
    std_msgs::msg::String src;
    src.data = "none";
    cmd_source_pub_->publish(src);
  }
  ...
  void onFullScanReceived(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    // タスク用 back_yaw へ向く（タスク完了後は従来の巡回に戻る）
    double back_yaw = M_PI/2.0;
    std_msgs::msg::Float32 msg_yaw;
    msg_yaw.data = back_yaw;
    heading_target_pub_->publish(msg_yaw);
  ...
  void onSecondScanTimer()
  {
    ...
    if (wait_counter_sec_ >= 30) {
      ...
      playVoice("再スキャンします。");
      requestFullScan();
      // この後に届く onFullScanReceived は「2回目」として扱う
    }
  }
  ...
  cv::Point2d computeLookaheadPoint()
  {
    ...
    if (dist_to_goal < lookahead_dist_*0.8) {
      best_pt = goal_;
      found = true;
    }
  ...
  void onTimer()
  {
    auto tnow = this->get_clock()->now();
    double dt = (tnow - last_pose_time_).seconds();

    if (dt > 0.5) {  // 0.5秒以上pose更新が無い
        pose_state_ = PoseSourceState::LOST;
    }
      ...
      if (!window_initialized) {
        ...
        int offset_y = 1080;   // 下側ディスプレイ想定
        cv::moveWindow("Manage Visualization", 100, offset_y + 100);
        window_initialized = true;
      }

      // DRIVE中のみゴールブロック検出（スキップ判定）
      if (phase_ == Phase::DRIVE) {
        if (isGoalBlocked()) {
          skipCurrentGoal();
          return;
      ...
      const cv::Point2d offset(W/2.0, H*0.75);
      ...
      switch (pose_state_) {
        case PoseSourceState::MEASURED:
          frame_color = cv::Scalar(0,255,0); // 緑
          break;

        case PoseSourceState::BLENDED:
          frame_color = cv::Scalar(255,128,0); // オレンジ（青寄せても良い）
          break;

        case PoseSourceState::PREDICTED:
          frame_color = cv::Scalar(0,255,255); // 黄（予測移動）
          break;

        case PoseSourceState::REJECT_KEEP_PREV:
          frame_color = cv::Scalar(255,0,0); // 青（外れ値リジェクト → 前回姿勢をそのまま使用）
          break;

        case PoseSourceState::LOST:
        default:
          frame_color = cv::Scalar(0,0,255); // 赤（自己位置喪失）
          break;
      }
      ...
      for (size_t i=0; i<goals_.size(); ++i) {
        cv::Scalar color = (i == (size_t)goal_index_) ?
                          cv::Scalar(0,0,0) : cv::Scalar(180,180,180);
      ...
      for (size_t i=0; i<marker_points_.size(); ++i) {
        ...
        cv::circle(canvas, toPix(m, scale, offset), 5, cv::Scalar(0,0,255), -1);
      ...
      // =====================================================
      // ① Pure Pursuit 本来の注視点（lookahead）を計算
      // =====================================================
      ...
      if (phase_ == Phase::DRIVE && !lidar_points_.empty()) {
          ...
          for (auto &p : lidar_points_) {
              double d = hypot(p.x - car_pos_.x, p.y - car_pos_.y);
              if (d < min_dist) {
                  min_dist = d;
                  nearest_pt = p;
              }
          ...
          if (d < (r1 + r2) && d > std::fabs(r1 - r2)) {
              ...
              intersections.push_back(cv::Point2d(P0.x + rx, P0.y + ry));
              intersections.push_back(cv::Point2d(P0.x - rx, P0.y - ry));
          }
          ...
          for (auto &p : intersections) {
              double dx = p.x - car_pos_.x;
              double dy = p.y - car_pos_.y;
              double angle = wrapToPi(std::atan2(dy, dx) - car_yaw_);
          ...
          // ★★★ 前方交点が1つも無い → 注視点は lookahead のまま ★★★
          if (front_inters.empty()) {
              selected_pt = lookahead;
          } else {

              // ★★★ ゴールに最も近い前方交点を選ぶ ★★★
              double best_dist = 1e9;
              for (auto &p : front_inters) {
                  double dist = hypot(p.x - goal_.x, p.y - goal_.y);
                  if (dist < best_dist) {
                      best_dist = dist;
                      selected_pt = p;
                  }
      ...
      // =====================================================
      // ④ DriveNode 用 target_point を publish（これが本丸）
      // =====================================================
  ...
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr pose_source_sub_;

  rclcpp::TimerBase::SharedPtr timer_;
  rclcpp::TimerBase::SharedPtr second_scan_timer_;

  PoseSourceState pose_state_ = PoseSourceState::LOST;
  rclcpp::Time last_pose_time_; 

// === full_scan の応答待ちフラグ ===
bool waiting_fullscan_ = false;

// === full_scan タイムアウト用タイマー ===
rclcpp::TimerBase::SharedPtr fullscan_timeout_timer_;
  ...
  // 2回目スキャン待ち
  bool waiting_second_scan_;
  int  wait_counter_sec_;

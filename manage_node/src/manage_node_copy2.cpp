...
private:
  ...
  void onLidar(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z) {
     ...
     // LiDAR の取り付けオフセット（base_link → robot_center）
      double lx = declare_parameter("lidar_offset_x_robot", 0.0);
      double ly = declare_parameter("lidar_offset_y_robot", 0.0);
      double x_robot = x_local + lx;
      double y_robot = y_local + ly;
